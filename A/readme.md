...
| Behavior | Accessible before declaration (value = undefined) | Cannot be accessed before declaration | Strong TDZ because it must be initialized immediately |
...
| Type | Scope | Hoisting | TDZ | Re-declaration | Re-assignment |
|------|-------|----------|-----|----------------|---------------|
| `var` | function-scoped | hoisted with undefined | no TDZ | Allowed | Allowed |
| `let` | block-scoped | TDZ applies | TDZ applies | Not allowed | Allowed |
| `const` | block-scoped | TDZ applies | TDZ applies | Not allowed | Not allowed |
...
3. `"End"` → Call Stack
4. After 2 seconds → callback goes to Callback Queue
...
### Why this happens:
This is a historical bug in JavaScript. In the early implementation, values were stored with type tags. `null` was incorrectly assigned the object type tag. It was never fixed due to backward compatibility.
...
`obj1` and `obj2` both point to the same memory location. So changing one affects the other.
